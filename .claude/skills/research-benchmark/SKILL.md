...
direction:             # supports | contradicts | neutral — CO rekord robi z tezą
...
**Kierunek dowodowy to nie to samo co wpływ na decyzję.** `direction` mówi, CO rekord robi z tezą; `decision_impact` mówi, CO ZMIENIA. Rekord `contradicts`, który każe wyciąć pół zakresu, ma ogromny wpływ na decyzję i jest dowodem uczciwości — nie potwierdzeniem tezy. Dlatego jednostronność wykrywa się na `direction`: alarm zapala się dopiero, gdy **wszystkie** poprawne rekordy mają `supports` i nie ma ani jednego `contradicts` czy `neutral`.
...
Rekord z `changes: ["none"]` **nie jest rozwijany w raporcie**. Trzy rekordy zmieniające decyzję są warte więcej niż dwanaście robiących wrażenie rzetelności. Research, który niczego nie zmienia, jest kosztem, nie dowodem — a `doublecheck()` uzna cały zestaw za ozdobnik, jeśli żaden poprawny rekord niczego nie zmienił. Uwaga: to jest test na `decision_impact`, nie na `direction` — te dwie kontrole są rozdzielone i mierzą co innego.
